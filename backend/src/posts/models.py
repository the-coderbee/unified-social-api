...
class Post(Base, TimestampMixin):
    ...
    status: Mapped[PostStatus] = mapped_column(
        SAEnum(PostStatus), default=PostStatus.PENDING
    )

    platform_results: Mapped[List["PostPlatformResult"]] = relationship(
        back_populates="post"
    )
...
class PostPlatformResult(Base, TimestampMixin):
    ...
    status: Mapped[PostResultStatus] = mapped_column(
        SAEnum(PostResultStatus), nullable=False
    )
