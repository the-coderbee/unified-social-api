...
from src.api.dependencies import get_current_user, RateLimiter
from src.api_keys.schemas import APIKeyCreate, APIKeyCreateResponse, APIKeyResponse
from src.api_keys.repository import create_api_key, get_api_keys_by_user, revoke_api_key_by_id, fetch_api_key_by_id
...
async def create_new_api_key(api_data: APIKeyCreate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    """
    ...
    try:
        ...
        new_api = await create_api_key(db=db, api_key_in=api_data, hashed_key=hashed_api_key, key_prefix=api_key_prefix, user_id=current_user.id)
...
async def fetch_all_keys(db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    """
...
async def revoke_api_key(api_key_id: uuid.UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    """
    ...
    try:
        ...
        if not api_key.user_id == current_user.id:
            raise HTTPException(
                ...
                detail=f"The API does not belong to you."
            )
        ...
        if not result:
            raise HTTPException(
                ...
                detail=f"Revoking API Key failed."
            )
