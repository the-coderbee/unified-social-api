...
async def create_api_key(db: AsyncSession, api_key_in: APIKeyCreate, hashed_key: str, key_prefix: str, user_id: uuid.UUID) -> APIKey:
    """
    Create new API Key.
...
async def get_api_keys_by_user(db: AsyncSession, user_id: uuid.UUID) -> List[APIKey]:
...
async def fetch_api_key_by_id(db: AsyncSession, api_key_id: uuid.UUID) -> Optional[APIKey]:
    """Fetch API Key by ID."""
